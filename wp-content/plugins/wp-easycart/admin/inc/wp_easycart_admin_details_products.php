	...
	public function basic_fields( ){
		...
		$fields = apply_filters( 'wp_easycart_admin_product_details_basic_fields_list', array(
			...
			array(
				...
				"visible"			=> true,
				"value"				=> $this->product->model_number,
				"message"			=> "<span id=\"sku_invalid_error\">SKU values must only include letters, numbers, and dashes</span><span id=\"sku_duplicate_error\" style=\"display:none;\">SKU is a duplicate and must be unique</span>"
			),
			array(
				"name"				=> "post_slug",
				"type"				=> "text",
	...
	public function pricing_fields( ){
		...
		$fields = apply_filters( 'wp_easycart_admin_product_details_pricing_fields_list', array(
			...
			array(
				...
				"placeholder"		=> 'Customize the login for pricing button label or leave empty for default'
			),
			array(
