...
class ec_db{
    ...
    public static function get_cart_count( $session_id ){
        $cart_array = self::$mysqli->get_results( self::$mysqli->prepare( "SELECT ec_tempcart.tempcart_id as cartitem_id FROM ec_tempcart WHERE ec_tempcart.session_id = '%s' AND ec_tempcart.session_id != '' AND ec_tempcart.session_id != 'deleted' AND ec_tempcart.session_id != 'not-set'", $session_id ) );
	...
	public static function insert_stripe_order( $subscription, $webhook_data, $user ){
		...
		if( isset( $webhook_data->discount ) && isset( $webhook_data->discount->coupon ) && isset( $webhook_data->discount->coupon->amount_off ) ){
			...
			$discount = round( $webhook_data->discount->coupon->percent_off * $webhook_data->subtotal / 100, 2 );
		}
